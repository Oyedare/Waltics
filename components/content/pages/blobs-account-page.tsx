...
const BlobsAccountPage = memo(function BlobsAccountPage() {
  const [address, setAddress] = useState("");
  const [page, setPage] = useState(0);
  const [size, setSize] = useState(20);

  const accountAddress = useMemo(() => address.trim() || null, [address]);

  const { data, isLoading, isError, refetch, isFetching } =
    useAccountBlobsPaginated(accountAddress, page, size);
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                disabled={!accountAddress || isFetching}
              >
                Fetch
              </button>
            </div>
            ...
            {!accountAddress ? (
              <div className="text-sm text-muted-foreground">
                Enter an account address to view blobs.
              </div>
            ) : (
                      ...
                      {isLoading ? (
                        ...
                        <tr>
                          <td
                            ...
                            className="px-4 py-8 text-center text-muted-foreground"
                          >
                            No blobs found for this account
                          </td>
                        </tr>
                      ) : (
                        rows.map((b) => (
